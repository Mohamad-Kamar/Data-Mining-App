...
def count_unique_values(data_set) -> dict:
    ...
    for entry in data_set:
        for element in entry:
            ...
            unique_values_count[element_as_tuple] = unique_values_count.get(
                element_as_tuple, 0) + 1
...
def get_unique_items(comb_freq_list, k):
    ...
    combination_iterable_of_sets = list(
        map(lambda element: set(element), combination_iterable))
    ...
    for combo in combination:
        for item_set in DATA_SET:
            if combo <= item_set:
                ...
                new_comb_freq[combo_tuple] = new_comb_freq.get(
                    combo_tuple, 0) + 1
...
def main():
    ...
    initial_combination_of_one = list(
        map(
            lambda element: set(element), initial_combination_frequency.keys()
        )
    )

    filtered_initial_combination_frequency = filter_combination_frequency_object(
        initial_combination_frequency, support_value)
    ...
    while len(combinations_frequency_list[-1]) != 0:
        ...
        current_filtered_comb_to_freq = filter_combination_frequency_object(
            current_comb_to_freq, support_value)
